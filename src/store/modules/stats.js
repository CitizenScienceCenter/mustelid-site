...
const actions = {
  updateTotalTaskCount({ state, commit, rootState }) {
    store
      .dispatch(
        "c3s/project/getProjectTasks",
        store.state.consts.identificationActivity
      )
      .then(res => {
        commit("SET_TOTAL_TASK_COUNT", res.body.msg.total);
      });
  },
  updateTotalUserAndSubmissionCount({ state, commit, rootState }) {
    const allSubmissionsQuery = {
      select: {
        fields: ["users.username", "count(*) as subs"],
        tables: ["submissions"],
      ...
      where: [
        {
          ...
          op: "i",
          val:
            "(select id from tasks where tasks.activity_id='" +
            store.state.consts.identificationActivity +
            "')",
          type: "sql"
        }
    ...
    store
      .dispatch("c3s/submission/getSubmissions", [allSubmissionsQuery, 99999])
      .then(res => {
        let allUsersCount = 0;
